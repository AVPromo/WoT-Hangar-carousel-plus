...
`VehicleFiltersDataProvider` is extended to accept row counts one through four and persists the selection in HCP's runtime state. The stock 2.3.1 renderer chunks only in pairs and uses equality checks for the double-row branch, so `patch-native-carousel.ps1` generalizes eight exact expressions in the locally installed bundle. The build verifies the source SHA and every replacement count. The generated patched bundle is packaged, but no Wargaming source bundle is stored in the repository.

## Sorting and last played

Custom ordering is disabled in 0.4.0. The Gameface provider enforces native ordering when no playlist is selected, and HCP no longer creates playlists. A future sorting implementation needs a separate provider-safe ordering hook.
